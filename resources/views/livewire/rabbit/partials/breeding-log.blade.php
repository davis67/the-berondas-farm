        ...
        @if ($selectedRabbit->logs != null)
            @foreach ($selectedRabbit->logs as $log)
                <li class="py-3 flex justify-between items-center">
                    ...
                    <button type="button" class="ml-6 bg-white rounded-md text-sm font-medium text-teal-600 hover:text-teal-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500">10<span class="sr-only"> Aimee Douglas</span></button>
                </li>
            @endforeach
        @else
            <li class="py-3 flex justify-between items-center">
            ...
            @if ($showMatingSlideover)
                <div class="flex items-center space-x-2 mt-4 md:mt-0" wire:click.away="{{ $showMatingSlideover = false }}" wire:keydown.escape.window="{{ $showMatingSlideover = false }}">
                    <div class="absolute w-80  text-left font-semibold bg-white shadow-dialog rounded-xl py-3 md:ml-8 bottom-0 md:bottom-10 right-8">
                        <form wire:submit.prevent="saveMatingRecord({{ $selectedRabbit->id }})" class="flex flex-col m-6 space-y-2">
