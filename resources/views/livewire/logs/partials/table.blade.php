<x-table>
    <x-slot name="head">
        ...
        <x-table.header sortable wire:click="sortBy('rabbit_no')" :direction="$sorts['rabbit_no'] ?? null">Sire </x-table.header>
        <x-table.header sortable wire:click="sortBy('rabbit_no')" :direction="$sorts['rabbit_no'] ?? null">Dam </x-table.header>
        <x-table.header sortable wire:click="sortBy('gender')" :direction="$sorts['gender'] ?? null">Date of Mating</x-table.header>
        ...
        @if ($selectPage)
            ...
            @forelse($logs as $log)
                <tr x-data @click="                                                       const target = $event.target.tagName.toLowerCase();                                                              const ignores = ['button', 'svg', 'path', 'a', 'input'];                                                               if(! ignores.includes(target))
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    }" class="rabbit-row @if ($selectedLog !=null && $log->id == $selectedLog->id) bg-teal-50 @endif bg-white hover:bg-gray-100 cursor-pointer"
                    wire:loading.class=" opacity-50">
                    <x-table.cell class="pr-0 hover:bg-gray-100">
                        <x-input.checkbox wire:model="selected" value="{{ $log->id }}" />
                    </x-table.cell>
                    ...
                    <x-table.cell>
                        <div class="flex">
                            <x-button.link class="view-rabbit text-sm" wire:click="selectLog({{ $log->id }})">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-teal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                                </x-button.primary>
                                <x-button.link class="text-sm" wire:click="handleSave({{ $log->id }})">
